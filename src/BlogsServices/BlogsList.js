...
//     const [accessDeniedOpen, setAccessDeniedOpen] = useState(false); // State for access restriction dialog
...
//     const handleToggleActive = async (blog) => {
//         // If the blog is inactive and the user is not an admin, show access denied dialog
//         if (!isAdmin && !blog.isActive) {
...
//             // Toggle active state
//             const updatedBlog = { ...blog, isActive: !blog.isActive };
//             await axios.patch(`http://localhost:5547/api/Blogs/${blog.id}/toggle-active`);
//             setBlogs(blogs.map((b) => (b.id === blog.id ? updatedBlog : b)));
//         } catch (error) {
//             console.error('There was an error updating the active status!', error);
//         }
...
//      // Function to close access restriction dialog
...
//         axios.patch(`http://localhost:5547/api/Blogs/${id}/toggle-active`)
//             .then(response => {
...
//         }
//         if (currentBlogs.id) {
//             axios.put(`http://localhost:5547/api/Blogs/${currentBlogs.id}`, BlogsToSave)
//             const res = await axios.get('http://localhost:5547/api/Blogs');
//             setBlogs(res.data);            
//         } else {
//             axios.post('http://localhost:5547/api/Blogs', BlogsToSave)
//             const res = await axios.get('http://localhost:5547/api/Blogs');
//             setBlogs(res.data);
//         }
//         setOpen(false);
...
//                 <h3 style={{ marginBottom: '20px', fontSize: '25px' }}>Blogs</h3>
//             </div>
...
//                                         // </IconButton>
//                                         <IconButton onClick={() => handleToggleActive(Blogs)} color="primary">
//                                             <UndoIcon />
//                                         </IconButton>
//                                     )}
//                                 </TableCell>
...
//             <Dialog open={accessDeniedOpen} onClose={handleAccessDeniedClose}>
//                 <DialogTitle>Access Denied</DialogTitle>
//                 <DialogContent>
//                     <Typography color="error">
//                         Only admins have access to activate inactive records.
//                     </Typography>
//                 </DialogContent>
//                 <DialogActions>
//                     <Button onClick={handleAccessDeniedClose} color="primary">OK</Button>
//                 </DialogActions>
//             </Dialog>
//         </div>
//     );
...
const handleToggleActive = async (id) => {
        try {
            const response = await axios.patch(`http://localhost:5547/api/Blogs/${id}/toggle-active`);
            setBlogs(blogs.map(blog =>
   ...
   const handleConfirmDialogClose = () => {
    ...
    const handleSave = () => {
        ...
        if (currentBlogs.id) {
            axios.put(`http://localhost:5547/api/Blogs/${currentBlogs.id}`, currentBlogs)
                .then(response => {
            ...
            axios.post('http://localhost:5547/api/Blogs', currentBlogs)
                .then(response => {
