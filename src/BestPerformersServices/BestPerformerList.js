...
function BestPerformersList({ isDrawerOpen }) {
    ...
    useEffect(() => {
        const fetchBestPerformers = async () => {
            try {
                const bestperformersResponse = await axios.get('http://localhost:5158/api/BestPerformers');
    ...
    const handleToggleActive = async (bestperformer) => {
        try {
            ...
            await axios.put(`http://localhost:5158/api/BestPerformers/${bestperformer.id}`, updatedBestPerformer);
    ...
    //     axios.patch(`http://localhost:5158/api/BestPerformers/${bestperformers.id}`)
    //         .then(response => {
    ...
    const handleDelete = (id) => {
        axios.patch(`http://localhost:5158/api/BestPerformers/${id}`)
            .then(response => {
                setBestPerformers(bestperformers.filter(b => b.id !== id));
            })
            .catch(error => {
                console.error('There was an error deleting the bestperformers!', error);
                setError(error);
            });
    ...
    const handleSave = () => {
        ...
        if (!currentBestperformers.frequency) {
            validationErrors.frequency = "Frequency is required";
        }
        if (!currentBestperformers.employeeID) {
            validationErrors.employeeID = "EmployeeID is required";
        }
        if (!currentBestperformers.clientID) {
            validationErrors.clientID = "ClientID is required";
        }
        if (!currentBestperformers.projectID) {
        ...
        const selectedEmployee = Employees.find(a => a.name === currentBestperformers.employeeID);
        const employeeId = selectedEmployee ? selectedEmployee.id : null;

        const selectedClient = Clients.find(a => a.name === currentBestperformers.clientID);
        const clientId = selectedClient ? selectedClient.id : null;
        const selectedProject = Projects.find(a => a.projectName === currentBestperformers.projectID);
        const projectId = selectedProject ? selectedProject.id : null;


        const BestperformersToSave = {
            ...currentBestperformers,
        ...
        if (currentBestperformers.id) {
            // Update request
            axios.put(`http://localhost:5158/api/BestPerformers/${currentBestperformers.id}`, BestperformersToSave)
                .then(response => {
            ...
            // Add request
            axios.post('http://localhost:5158/api/BestPerformers', BestperformersToSave)
                .then(response => {
    ...
    const handleChange = (e) => {
        ...
        if (name === "clientID") {
            if (value) {
                setErrors((prevErrors) => ({ ...prevErrors, clientID: "" }));
            }
        }
        if (name === "projectID") {
            if (value) {
                setErrors((prevErrors) => ({ ...prevErrors, projectID: "" }));
            }
        }
    };
    ...
    const handleClose = () => {
        ...
        setErrors({employeeID: '', frequency: '', clientID: '', projectID: ''})
